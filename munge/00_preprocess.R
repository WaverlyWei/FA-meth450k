...
raw_data <- here("Superfund/data", "raw_data.RData")

# only load data from raw files if raw_data does not exist
if (!file.exists(normalizePath(raw_data))) {

  print("raw_data object not found; attempting to read raw files...")

  load(here("SuperFund/data", "rgsetAll.Rdata"))
  # ERROR: faile to load 
  RGset_raw <- rgsetAll
...
# remove FALSE columns 
  ...
  RGEffect=TRUE, Rplot=FALSE, resultsDir=here::here("SuperFund/graphs", "QC",
  ...
  pdf(file = here::here("SuperFund/graphs","QC", "PotentialOutliers.pdf"))
  plotQC(QC_minfi$qc)
  dev.off()
  ...
  # Filtering probes with a detection p-value above 0.01.
  # Removing 12655 probes.
  # If a large number of probes have been removed, ChAMP suggests you to identify potentially bad samples
  # 
  # Filtering NoCG Start
  # Only Keep CpGs, removing 2663 probes from the analysis.
  # 
  # Filtering SNPs Start
  # Using general 450K SNP list for filtering.
  # Filtering probes with SNPs as identified in Zhou's Nucleic Acids Research Paper 2016.
  # Removing 57104 probes from the analysis.
  # 
  # Filtering MultiHit Start
  # Filtering probes that align to multiple locations as identified in Nordlund et al
  # Removing 11 probes from the analysis.
  ...
  # [ Section 2: Filtering Done ]
  # 
  # All filterings are Done, now you have 403610 probes and 67 samples.
  # 
  # [<<<<< ChAMP.FILTER END >>>>>>]
  # [===========================]
  # [You may want to process champ.QC() next.]
  ...
  write.csv(pheno, file=here("SuperFund/data", "pheno_CGR.csv"))
  write.csv(pheno_P, file=here("SuperFund/data", "pheno_P.csv"))
  write.csv(pheno_PS, file=here("SuperFund/data", "pheno_PS.csv"))
